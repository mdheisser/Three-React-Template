...
const AnimationWidget = () => {
  ...
  return (
    <>
      {time.custom ? (
        <span className="overlay right" id="timeinfo">
...
const Main = () => {
  ...
  useEffect(() => {
    ...
    var tmax = 24 * 60;
    mat.makeTranslation(
      (statBoxCenter.x / 2) * (1 + Math.cos((2 * Math.PI * t) / tmax) * 2),
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "keydown",
      evt => {
        if (evt.code === "Space") anim.current = !anim.current;
      },
      false
    );
  }, []);
  ...
  const helpers = [
    <BoxEntityCtrlHlp
      key={0}
      boxEnt={staticBoxEnt}
      boxStyle={StaticBoxStyle}
    />,
    <BoxEntityCtrlHlp
      key={1}
      boxEnt={movingBoxEnt}
      onChange={onMove}
      boxStyle={MovingBoxStyle}
    />
  ];
  ...
  const splitBoxHelpers = splitBoxes.map((box: Box3, i: number) => {
    ...
    return (
      <BoxEntityCtrlHlp key={i + 2} boxEnt={ent} boxStyle={{ ...style }} />
    );
  });
